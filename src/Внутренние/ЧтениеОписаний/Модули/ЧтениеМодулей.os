...
Процедура ДополнитьБлокиМодуля(БлокиМодуля, Файл, Модуль)
	...
	Для Каждого Блок Из БлокиМодуля Цикл
		...
		ИначеЕсли Блок.ТипБлока = ТипыБлоковМодуля.ЗаголовокПроцедуры
			...
			ЗаполнитьОписаниеМетода(Блок, ПоследнийБлокКомментария, Файл);
...
Процедура ЗаполнитьОписаниеМетода(Блок, ПоследнийБлокКомментария, Файл)
	...
	Если СтрНайти(СтрокаКомментария, Блок.ОписаниеБлока.ИмяМетода) Тогда
		началоОписания = 1;
	Иначе
		началоОписания = 0;
	КонецЕсли;
	...
	Для Ит = ПоследнийБлокКомментария.НачальнаяСтрока + началоОписания По ПоследнийБлокКомментария.КонечнаяСтрока Цикл
